...
class DiagDataSourceImp implements DiagnosisDataSource {
  @override
  Future<Either<Failure, List<DiagnosisModel>>> getDiagnosis() async {
    try {
      var choices=<DiagnosisModel>[];
      var map={
  "itching": 0,
  "skin_rash": 0,
  ...
  "yellow_crust_ooze": 0
      ...
      for (var element in map.keys) {
          ...
          choices.add(DiagnosisModel(element));
  ...
  @override
  Future<Either<Failure, String>> predict() async {
    var cubit = Modular.get<DiagnosisCubit>();
    var map = {};
    cubit.diagnosis?.forEach((element) {
      map[element.key] = element.isSelected ? 1 : 0;
    });
    Dio dio = Dio();
    dev.log(map.toString());
    var response = await dio.post('http://10.0.2.2:8080/predict',
